...
export default function DetCategoria({ navigation }) {
  ...
  const title_get= route.params.nombre.toString()
    ...
    useEffect( () => {
      onSnapshot(collection(db,"producto"), (querysnapshot) => {
        ...
        querysnapshot.forEach((doc) => {
          const {nombre, descripcion, imagen, categoria}= doc.data();

            

              console.log("funciona el get", categoria, title_get);
  ...
  const renderRecipes = ({item}) => (
    ...
    <TouchableHighlight underlayColor="#ffecb3" onPress={()=> navigation.navigate("DetPedido", item)}>
